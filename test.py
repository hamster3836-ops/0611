...
# =============================================================================
with tab1:
    st.markdown('<div class="step-title">1단계: 측정 데이터 관찰 및 시각화 미션</div>', unsafe_allow_html=True)
    st.write("환자의 혈액을 채취하여 측정한 [시간별 혈중 약물 농도 데이터]입니다.")
    
    # 💡 [변경 포인트] 데이터 프레임을 먼저 제시
    st.dataframe(df_sample.T, use_container_width=True) # 전치(T)를 사용하여 가로로 길게 보여줌
    
    st.markdown("### 📊 [미션 1] 데이터를 산점도로 시각화하기")
    st.markdown('<div class="mission-box">', unsafe_allow_html=True)
    st.write("이 데이터를 그래프로 그리려면 **독립변수(원인)**와 **종속변수(결과)**를 축에 알맞게 배치해야 합니다. X축과 Y축에 들어갈 알맞은 데이터를 선택해 보세요.")
    
    col_x, col_y = st.columns(2)
    with col_x:
        x_axis = st.selectbox("X축 (독립변수):", ["선택하세요", "Time(h)", "Conc(mg/L)"])
    with col_y:
        y_axis = st.selectbox("Y축 (종속변수):", ["선택하세요", "Time(h)", "Conc(mg/L)"])
    st.markdown('</div>', unsafe_allow_html=True)

    is_plot_ready = False
    
    if x_axis == "Time(h)" and y_axis == "Conc(mg/L)":
        st.success("🎉 정답입니다! 시간이 지남에 따라 농도가 어떻게 변하는지 보여주는 산점도가 완성되었습니다.")
        is_plot_ready = True
    elif x_axis != "선택하세요" and y_axis != "선택하세요":
        st.error("앗! 축 설정이 잘못되었습니다. 시간의 흐름(원인)에 따라 농도(결과)가 변하는 것이므로, 무엇이 X축에 가야 할지 다시 생각해 보세요.")

    # 미션 1을 통과해야만 그래프와 2단계 탐구가 열림
    if is_plot_ready:
        fig_data = go.Figure()
...
with tab2:
    ...
    st.markdown("### 🧩 [미션 2] 나만의 지수함수 모델 완성하기")
    st.markdown('<div class="mission-box">', unsafe_allow_html=True)
    ...
    if is_model_correct:
        st.markdown("---")
        st.markdown("### ✍️ [미션 3] 약물 반감기($t_{1/2}$) 직접 계산하기")
